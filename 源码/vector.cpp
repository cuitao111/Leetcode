...
namespace cth {
	class NoCopy {
		public:          inline NoCopy() {
		}
		NoCopy(const NoCopy&);
		NoCopy& operator=(const NoCopy&);
	}
	;
	template<typename T>     class viterator:public std::iterator<std::forward_iterator_tag,T> {
		public:          viterator() {
			t=NULL;
		}
		viterator(T* t_) {
	...
	;
	template<typename T>     class cvector:public NoCopy {
		public:          typedef viterator<T> iterator;
		//viterator<T>就是对一个指针的包装，所以完全可以用T*代替
		viterator <T>         typedef const viterator<T> const_iterator;
		...
		cvector(int capa,const T& val=T()) {
			...
			for (int i=0;i<size_;i++)                  buf[i]=val;
		}
		cvector(const_iterator first,const_iterator last) {
			...
			int index=0;
			while(iter!=last)                 buf[index++]=*iter++;
		}
		~cvector() {
			if (buf) {
				delete[] buf;
		...
		iterator erase(const_iterator first,const_iterator last) {
			...
			while(first_<=last_--)                  erase(first_);
		...
		private:          void newCapacity(int capa) {
			capacity_=capa;
